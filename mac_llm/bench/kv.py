...
class IncompatibleCacheError(Exception):
    """Raised when restored cache metadata does not match the expected context."""
...
class PromptCacheStore(Protocol):
    def save(self, payload: bytes, metadata: CacheMetadata) -> tuple[int, float]: ...
...
def estimate_prefix_tokens(prefix_text: str) -> int:
...
def build_metadata(
    *,
    ...
    return CacheMetadata(
        ...
        prefix_tokens=estimate_prefix_tokens(prefix_text),
    )
...
def run_kv_benchmark_cli(
    *,
    ...
    return run_kv_benchmark(
        prefix_name=prefix_name,
        metadata=metadata,
        cache_store=KVPromptCacheStore(cache_root),
